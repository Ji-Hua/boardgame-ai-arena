...
Agent definitions live in `agent_system/definition/agent_defs/` (the canonical
source shared by both Arena and Agent Service). The Arena loads definitions
through the shared `AgentSpec` model (`agent_system.definition.agent_spec`)
and materializes Arena-specific runtime objects (Scorer + Policy → Agent).
...
```yaml
# agent_system/definition/agent_defs/minimax_d3.yaml
id: minimax_d3
